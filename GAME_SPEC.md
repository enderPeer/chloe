...
2. The ceiling is **per knight**, `capForKnight` = his opening level + `overCap`, not the round baseline + `overCap`. Under §28's ceiling every knight in a long round-5 fight converged on level 7 and the ladder evaporated precisely when the fight had run long enough to matter. Measured in a real round-5 fight: spawn `[5,5,3,2,2]` climbing to `[7,7,5,4,4]`, where §28's rule gave `[7,7,7,7,7]`. The veteran's ceiling is unchanged from §28 (round + overCap, because his opening level IS the round).
